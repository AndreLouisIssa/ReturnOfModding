		...
		if ImGui.IsItemHovered() then
			...
			if value_part then
				if sd.type == "string" then
					...
					if message.type == RValueType.STRING then message = message else message = nil end
				end
			end
			if message == nil then
				...
				elseif type(dd.data.class_name) == "string" then
					...
					enum = hardcoded.array[enum][hardcoded.enum[enum][sd.show]]
				end
